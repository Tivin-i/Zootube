...
- **Migrations squashed into one.** All database migrations are now a single file: `migrations/001_schema.sql`. It includes everything from the former 001–004 (schema, RLS, triggers, `ensure_parent_from_auth_email`, service_role grants on households, household_members, device_tokens, parents, and EXECUTE on helper functions). Run it once in the Supabase SQL Editor; idempotent. Removed `002_service_role_grants.sql`, `003_ensure_parent_from_auth.sql`, `004_service_role_parents.sql`. `docs/setup.md` updated to describe the single migration.

- **Phase 4: Verification and final code review.** (1) **Verification Loop:** Ran `npm run build`, `npx tsc --noEmit`, `npm run lint`, and unit tests; fixed TypeScript regression in `__tests__/lib/utils/error-handler.test.ts` (NODE_ENV read-only: use `Object.defineProperty` instead of assignment). (2) **Code Reviewer:** Reviewed all modified files (device-token, parent-by-email, auth callbacks, validate-invite, children/by-device, parent.service, admin.ts, feed, link-device, watch, AdminDashboard, KidsHeader, docs, migrations). No Critical or High issues; approval with minor suggestions (parent.service fallback logging, comments). Report: `docs/reviews/phase4-verification-and-code-review.md`. (3) **Test coverage:** `npm run test:coverage` was failing with babel-plugin-istanbul/test-exclude (incompatible with `glob@12` override). Set `coverageProvider: 'v8'` in `jest.config.js` so coverage runs without Istanbul; all 4 suites (32 tests) pass; global coverage thresholds may still fail until more code is covered. (4) **E2E:** Ran `npm run test:e2e` (27 Playwright tests). Some tests fail when the app uses a DB without migrations (e.g. `videos.household_id` missing) or when rate limits are hit; ensure migrations are applied and consider a dedicated E2E env or higher limits for local E2E.

- **Phase 3 refactor and security fixes.** (1) **Refactor:** Removed unused files `lib/hooks/useParentId.ts`, `lib/validators/api-responses.validator.ts`, `lib/validators/database.validator.ts`; added `docs/DELETION_LOG.md`. (2) **High:** POST `/api/device-token` now requires an authenticated session and enforces `parentId === user.id` (403 otherwise); device-token service comment notes production requires `SUPABASE_SERVICE_ROLE_KEY`. (3) **Medium:** New migration `004_service_role_parents.sql` grants service_role on `public.parents` for admin fallback; admin client rejects placeholder URL/key like server client; validate-invite returns `valid: false` in production when no invite codes are set; rate-limit comment documents trusted-proxy requirement; parent service comment documents listUsers(1000) cap. (4) **Low:** `docs/setup.md` documents migration order (001 → 003, 002 if needed, 004) and production requirement for `SUPABASE_SERVICE_ROLE_KEY`; YouTube and child OAuth callbacks log error message (no secrets) on failure. (5) **Web/a11y:** Link-device success SVG and feed empty-state SVG get `aria-hidden="true"`; admin login/signup error and success blocks get `aria-live="polite"` and signup error `role="alert"`; signup button loading text "Creating account..." → "Creating account…".

- **Phase 2 review: consolidated findings and refactor plan.** Merged Phase 1 reports (architecture, security, database, tests, logic) into `docs/reviews/phase2-consolidated-findings.md` with prioritized Critical/High/Medium/Low list. Ran knip, depcheck, ts-prune for unused files, exports, and dependencies. Applied Web Interface Guidelines to link-device, feed, watch, and admin UI pages. Produced `docs/reviews/phase2-refactor-cleanup-plan.md` with concrete actions (what to remove, consolidate, fix) and file references for Phase 3 implementation. No code changes in Phase 2.

### Changed

- **Set up device / feed: no default "Zoe" child; use linked children or redirect.** The kid-facing feed and watch pages no longer show a hardcoded "Zoe". (1) New API `GET /api/children/by-device` returns the list of linked children (household_children) for the household identified by the device token cookie. (2) Feed page: if the household has no linked children, it shows "No profiles yet" and a link to the admin so the parent can add child accounts (Google OAuth). (3) If there are linked children, the feed shows the selected child’s name in the header and, when multiple children exist, a profile switcher in the header menu. Selection is persisted in localStorage and used on the watch page. (4) Hook `useChildrenByDevice` and constants `SELECTED_CHILD_ID_KEY` / `SELECTED_CHILD_NAME_KEY` added. KidsHeader accepts optional `children`, `selectedChildId`, and `onSelectChild` for the profile switcher.

- **Household model: every parent has one household or is invited.** (1) Removed the "Create household" form from the admin dashboard. When a parent has no households, the app now automatically creates one ("My list") on first load; if that fails, a "Retry" button is shown. (2) Parent-by-email API ensures a default household exists when a parent is found but has none (e.g. backfill from auth). (3) Link-device copy no longer mentions "create a household"; it now says "Sign in to the admin to finish setup" and "This account isn’t set up yet. Sign in to the admin to finish setup, then try linking again."

### Fixed

- **Create household: "permission denied for table households":** The service role key needs explicit table grants. Migration `001_schema.sql` now grants `service_role` SELECT/INSERT/UPDATE/DELETE on `households`, `household_members`, and `device_tokens`. For existing databases, run `migrations/002_service_role_grants.sql` in the Supabase SQL Editor.

- **Set up device: "No account found with this email":** (1) Parent lookup is case-insensitive (ILIKE). (2) When the parent is missing from `public.parents` but exists in `auth.users` (e.g. signup before trigger), the API backfills via `ensure_parent_from_auth_email`: syncs parent row and creates default household and membership. Run `migrations/003_ensure_parent_from_auth.sql` in Supabase SQL Editor if your DB was created before this change. (3) Link-device page shows a distinct message when the account exists but has no household.
